...
class Application(Frame):
    """Guess My Number (The GUI application)."""
    ...
    def check_guess(self, guess):
        ...
        if guess < MIN or guess > MAX:
            self.display_message("Invalid Input, Guessed Out Side Of Range.")
    ...
    def reset(self):
        ...
        self.display_message(
            "Game Reset. Please enter another number to play again.")
        self.tries = 0
        Label(self,
              text="Number Of Tries: " + str(self.tries)
              ).grid(row=0, column=2, columnspan=1, sticky=W)
    ...
    def resetgame(self):
        self.display_message("Congrats! You guessed correctly. The number was " +
                             str(self.number) + ". And it only took your " +
